  ...
  if (Serial.available()) {  // Check if data is available to read
    ...
    switch(command) {
      ...
      case 'B':  // Move backward
        motor1.run(100);  // Full speed backward for motor1
        motor2.run(-100);  // Full speed backward for motor2
        break;
      case 'L':  // Turn left
        motor1.run(120);  // Reverse motor1 to turn left
        motor2.run(120);   // Forward motor2 to turn left
        break;
      case 'R':  // Turn right
        motor1.run(-120);   // Forward motor1 to turn right
        motor2.run(-120);  // Reverse motor2 to turn right
        break;
      case 'S':  // Stop
